...
def run_sampling(packets_per_file, packets_per_connection, duration_seconds):
    ...
    subprocess.run(["./samp.exe", packets_per_file, packets_per_connection, duration_seconds])
...
def connection_analysis():
    ...
    # miner_log_path = r'C:\Users\Andreas\Desktop\tmaproject\analyzed\miner_connection.log'  # Adjust path as needed
    # warning_log_path = r'C:\Users\Andreas\Desktop\tmaproject\analyzed\warning_connection.log'  # Adjust path as needed

    miner_connections = parse_log(miner_log_path)
    warning_connections = parse_log(warning_log_path, warning=True)

    return render_template('track.html', miner_connections=miner_connections, warning_connections=warning_connections)

@app.route('/track_connection', methods=['POST'])
def track_connection():
    ip1 = request.form['ip1']
    ip2 = request.form['ip2']
    # Call track.exe with the IPs
    subprocess.run(['evalioation.exe', ip1, ip2], shell=True)
